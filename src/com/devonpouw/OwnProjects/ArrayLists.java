...
//         to know about 7 to 10:
        ...
        System.out.println("You've got the first prize: " + prizes.get(0));

        // Find the error 3: how to fix this?
        System.out.println("The last word in the sentence is '" + sentence.get(sentence.size() - 1) + "'.");

        // Fill in the missing code (1 of 3) ** er stond println ipv printf. Aantal minuten bezig geweest tot ik daar achter kwam
        List<String> japaneseNumbers = new ArrayList<>(List.of("rei", "ichi", "ni", "san", "yoku", "go", "roku", "nana", "hachi", "kyuu", "juu"));
        System.out.printf("I know %d Japanese numbers!\n", japaneseNumbers.size());

        // Fill in the missing code (2 of 3)
        System.out.print("Which digit do you want to know the Japanese word for? ");
        int digit = in.nextInt();
        System.out.printf("%d in Japanese is '%s'.", digit, japaneseNumbers.get(digit - 1));

        // Fill in the missing code (3 of 3)
        japaneseNumbers.add("juuichi");
        japaneseNumbers.add("juuni");
        System.out.println("\nThe Japanese numbers I know are: ");
        ...
        ArrayList<Double> presidentHeights = new ArrayList<>();
        ArrayList<String> presidentNames = new ArrayList<>();
        presidentNames.add("Lincoln");
        presidentNames.add("Reagan");
        presidentNames.add("Madison");
        presidentNames.add("Hayes");
        presidentNames.add("Biden");
        presidentHeights.add((1.93)); //Lincoln
        presidentHeights.add((1.85)); //Reagan
        presidentHeights.add((1.63)); //Madison
        presidentHeights.add((1.73)); //Hayes
        presidentHeights.add((1.83)); //Biden

        // Part 2:
        // print out all the heights

//        for (Double heights : presidentHeights) {
//            System.out.println(heights);
//        }

        for (int i = 0; i < presidentHeights.size(); i++) {
            System.out.print(presidentNames.get(i) + height);
            for (int j = 0; j < 1; j++) {
                System.out.println(presidentHeights.get(i));
            }
        }
        ...
        System.out.println("Amount of president heights in the list: " + presidentHeights.size());
        // Part 4: Calculate the average height of your presidential sample
        double totalHeight = 0;
        for (int i = 0; i < presidentHeights.size(); i++) {
            totalHeight += presidentHeights.get(i);
        }
        double averageHeight = totalHeight / presidentHeights.size();
        System.out.println("Average height of the presidents are: " + averageHeight);
        // Part 5: Add joe Biden's height (1.83)
//        presidentHeights.add(1.83);
        // Part 6: Again print out all heights, the number of heights, and the average height
//        for (Double presidentHeight : presidentHeights) {
//            average += presidentHeight;
...
//        System.out.println(averageHeight);
        ...
        System.out.println("Is there a president with length 1.80? " + presidentHeights.contains(1.80));
        System.out.println("Is there a president with length 1.85? " + presidentHeights.contains(1.85));
        // Part 8: Show the index of the candidate with length 1.63
        System.out.println("The index of the president with height 1.63 is: " + presidentHeights.indexOf(1.63));
    }
}
